...
import { createYahtzeeGame } from '../api'
import { Button, Card, LoadingIcon, Page } from '../components'

interface YahtzeeHomeProps {}

export const YahtzeeHome: React.FC<YahtzeeHomeProps> = ({}) => {
    ...
    const onHost = () => {
        ...
        setHost(true)
        createYahtzeeGame(user.uid, user.name, gid => navigate(`/play/yahtzee/${gid}`))
    }

    const onJoin = () => {
        navigate(`/play/yahtzee/${code}`)
    }

    return (
        <Page centered>
            <div className='w-full max-w-4xl h-full p-5 flex-col gap-5 flex'>
                <h1 className='text-3xl font-extrabold'>Play Yahtzee</h1>
